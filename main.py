...
async def on_startup(bot: Bot) -> None:
    ...
    # ✅ HIGH-5: загружаем health для всех email-аккаунтов из БД
    try:
        from services.anti_ban import load_health_from_db
        from database.models import EmailAccount
        async with async_session() as sess:
            accs = list(await sess.scalars(select(EmailAccount)))
        for acc in accs:
            await load_health_from_db(acc.email, acc.user_id)
        logger.info("EmailHealth loaded for %d accounts", len(accs))
    except Exception as e:
        logger.warning("Failed to load EmailHealth: %s", e)
...
async def main() -> None:
    ...
    async def cmd_start(message: Message):
        ...
        from keyboards.settings_inline import MESSAGES, get_settings_menu_keyboard
        from database.engine import async_session as as2
        async with as2() as sess2:
            u = await sess2.scalar(select(UserSettings).where(UserSettings.user_id == message.from_user.id))
            if not u:
                from database.models import UserSettings as US
                u = US(user_id=message.from_user.id)
                sess2.add(u)
                await sess2.commit()
