...
async def proxy_check_all(callback: CallbackQuery, session: AsyncSession):
    ...
    await session.commit()
    await msg.edit_text(MESSAGES["proxy_check_done"].format(alive=alive, dead=dead))
...
# ─── Loma Proxy API: автозагрузка прокси ────────────────────────────────────

@proxies_router.callback_query(F.data == "proxy_loma_import")
async def loma_import_menu(callback: CallbackQuery, state: FSMContext):
    """Меню импорта прокси из Loma Proxy API (CRIT-15)."""
    from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton
    await state.set_state(ProxyAddState.waiting_for_loma_api_key)
    await callback.message.edit_text(
        "🌐 <b>Импорт из Loma Proxy API</b>\n\n"
        "Введите ваш API-ключ Loma (получить: https://dashboard.lomaproxy.com):\n\n"
        "<i>Будут загружены статические SOCKS5-прокси и добавлены в ваш список.</i>",
        parse_mode="HTML",
...
async def loma_import_process(message: Message, state: FSMContext, session: AsyncSession):
    """Обрабатывает API-ключ Loma и импортирует прокси."""
    from services.loma_proxy import get_loma_service
    api_key = message.text.strip()
    await message.delete()  # удаляем API-ключ из чата

    if len(api_key) < 10:
        await message.answer("❌ Слишком короткий API-ключ.")
        await state.clear()
        return

    status_msg = await message.answer("⏳ Импорт прокси из Loma API...")
    await state.clear()

    loma = get_loma_service()
    added, failed = await loma.import_static_to_db(
        user_id=message.from_user.id,
        api_key=api_key,
        proxy_type="socks5",
        limit=100,
    ...
    await status_msg.edit_text(
        ...
        f"Добавлено: <b>{added}</b> прокси\n"
        f"Пропущено (дубликаты): <b>{failed}</b>",
        parse_mode="HTML",
    )
...
async def loma_rotating_menu(callback: CallbackQuery, state: FSMContext):
    """Меню добавления rotating backconnect-прокси Loma (gate.lomaproxy.com)."""
    from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton
    ...
    await callback.message.edit_text(
        ...
        "<i>(username обычно имеет формат: user-country-de-session-XYZ)</i>",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="🔙 Назад", callback_data="back_settings"),
        ]]),
...
async def loma_rotating_process(message: Message, state: FSMContext, session: AsyncSession):
    ...
    loma = get_loma_service()
    added = await loma.import_rotating_to_db(
        user_id=message.from_user.id,
        username=username,
        password=password,
        proxy_type="socks5",
    ...
    if added:
        ...
        await message.answer("ℹ️ Rotating-прокси уже есть в списке.")
