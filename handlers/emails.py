...
async def emails_menu(callback: CallbackQuery, session: AsyncSession):
    ...
    await callback.message.edit_text(
        MESSAGES["emails_menu"].format(count=len(emails)),
        parse_mode="HTML",
        reply_markup=get_emails_menu_keyboard(len(emails), page=0),
    )
...
async def email_select(callback: CallbackQuery, session: AsyncSession):
    ...
    if not emails:
        ...
        return
    await callback.message.edit_text(
        "📩 <b>Список E-mail для рассылки:</b>",
        parse_mode="HTML",
        reply_markup=get_emails_list_keyboard(emails, page=0),
    )
    await callback.answer()
...
async def emails_paginate(callback: CallbackQuery, session: AsyncSession):
    ...
    await callback.message.edit_text(
        "📩 <b>Список E-mail для рассылки:</b>",
        parse_mode="HTML",
        reply_markup=get_emails_list_keyboard(emails, page=page),
    )
    await callback.answer()
...
async def email_detail(callback: CallbackQuery, session: AsyncSession):
    ...
    if not account:
        ...
        return
    await callback.message.edit_text(
        f"📧 <b>{account.display_name or account.email}</b>\nE-mail: <code>{account.email}</code>\nСтатус: {'✅ Валиден' if account.is_valid else '❌ Не валиден'}",
        parse_mode="HTML", reply_markup=get_email_detail_keyboard(email_id),
    )
    await callback.answer()
...
async def email_delete_confirm(callback: CallbackQuery, session: AsyncSession):
    ...
    if account:
        ...
        await session.commit()
    emails = list(await session.scalars(select(EmailAccount).where(EmailAccount.user_id == callback.from_user.id).order_by(EmailAccount.created_at)))
    await callback.message.edit_text(MESSAGES["emails_menu"].format(count=len(emails)), parse_mode="HTML", reply_markup=get_emails_menu_keyboard(len(emails)))
...
async def email_delete_all(callback: CallbackQuery, session: AsyncSession):
    ...
    await session.commit()
    await callback.message.edit_text(MESSAGES["emails_menu"].format(count=0), parse_mode="HTML", reply_markup=get_emails_menu_keyboard(0))
