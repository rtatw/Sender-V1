...
async def _handle_json_content(
    ...
    for idx, item in enumerate(items_list):
        ...
        # ✅ NEW: сохраняем item_id для дедупликации по объявлению
        # (kleinanzeigen.de item_id = цифры в конце item_link)
        item_id = str(item.get("item_id") or "")
        if not item_id and link:
            # Извлекаем item_id из ссылки как fallback
            import re as _re_id
            m = _re_id.search(r"/(\d+)(?:$|/|\?)", link)
            if m:
                item_id = m.group(1)
