...
from database.models import UserSettings, EmailAccount, IncomingMessage, ReceiveEmail
...
class InboxWatcher:
    ...
    async def _watch(self, user_id, chat_id):
        while True:
            ...
            # Интервал берём из настроек пользователя
            try:
                async with async_session() as s:
                    st = await s.scalar(select(UserSettings).where(UserSettings.user_id == user_id))
                    interval = st.receive_check_interval if st else 30
            except Exception:
                interval = 30
