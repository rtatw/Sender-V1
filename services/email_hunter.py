...
def _make_variations(person_name: str) -> list[str]:
    """Создаёт варианты email-адресов из имени человека.

    Расширенный набор вариаций (раньше было только 2):
      Для "Daniel Herdt":
        - danielherdt          (concat)
        - daniel.herdt         (dot)
        - daniel_herdt         (underscore)
        - daniel-herdt         (hyphen)
        - d.herdt              (initial + last)
        - herdt.daniel         (last.first)
        - herdt.d              (last + initial)

      Для "Sternchen" (одно слово):
        - sternchen

    Также если есть цифры — дополнительно добавляем вариацию без цифр.
    Фильтр: длина >= 5 символов, уникальные.
    """
    ...
    def add(v: str):
        v = v.lower().strip("._-")
        # Только длиной >= 5 (Gmail требует 6+, но мы мягче — для коротких доменов)
        if v not in seen and len(v) >= 5:
            seen.add(v)
            result.append(v)

    if len(parts) == 1:
        # Одно слово — только оно само
        add(parts[0])
    else:
        # Берём первые 2 значимых слова (имя + фамилия)
        f, s = parts[0].lower(), parts[1].lower()
        # ✅ Расширенный набор разделителей
        add(f + s)              # danielherdt
        add(f + "." + s)        # daniel.herdt
        add(f + "_" + s)        # daniel_herdt
        add(f + "-" + s)        # daniel-herdt
        # ✅ Инициал + фамилия: d.herdt
        if len(f) >= 1:
            add(f[0] + "." + s)
            add(f[0] + s)       # dherdt
        # ✅ Фамилия + имя
        add(s + "." + f)        # herdt.daniel
        add(s + f)              # herdtdaniel
        # ✅ Фамилия + инициал: herdt.d
        if len(f) >= 1:
            add(s + "." + f[0])

    # Если есть цифры — добавляем вариант без цифр (часто год рождения/номер)
...
class EmailHunter:
    ...
    async def _do_hunt(self, session: AsyncSession) -> None:
        ...
        # ✅ NEW: Сортируем items по item_id, чтобы все вариации одного
        # объявления шли подряд. Тогда можно прерывать проверку как только
        # нашли первый email для этого item_id.
        items.sort(key=lambda it: (it.item_id or "", it.nickname))
        # Group items by item_id
        from itertools import groupby
        items_by_ad = []
        for item_id, group in groupby(items, key=lambda it: it.item_id or ""):
            items_by_ad.append((item_id, list(group)))

        # Уникальные объявления (по item_id или по person_name если item_id пустой)
        unique_ads = len({it.item_id or it.person_name for it in items})
        display_total = unique_ads if unique_ads > 0 else total
        await self._update_message(
            f"🔍 <b>Email Hunter</b>\n\n"
            f"📧 Объявлений: {display_total}\n"
            f"📝 Вариаций: {total}\n"
            f"🌐 Прокси: {len(proxy_configs)}\n"
            f"⚡ Запуск..."
        )

        from services.email_verifier import verify_email as verify_email_unified
        ...
        async def _process_ad(ad_id: str, ad_items: list) -> bool:
            """Обрабатывает ОДНО объявление со всеми его вариациями.

            Возвращает True если email найден.
            Логика:
              1. Берём первый item из группы (вариации).
              2. Для каждого item пробуем все домены (gmail, mail.ru, ...).
              3. Если нашли email — помечаем ВСЕ остальные items этого ad_id
                 как DONE без проверки (дедупликация по объявлению).
              4. Если не нашли — помечаем все items как DONE без email.
            """
            async with sem:
                if self._cancelled:
                    return False
                found_email = ""
                # Пробуем каждую вариацию, пока не найдём валидный email
                for item in ad_items:
                    if self._cancelled:
                        return False
                    if found_email:
                        # Уже нашли — помечаем остальные items того же ad как DONE
                        item.status = ItemStatus.DONE
                        item.found_email = ""
                        continue
                    for domain in ordered_domains:
                        try:
                            email_to_check = f"{item.nickname}@{domain}"
                            result = await verify_email_unified(
                                email_to_check,
                            ...
                            if result.get("exists") and result.get("code") == "ok":
                                item.found_email = email_to_check
                                item.status = ItemStatus.DONE
                                found_email = email_to_check
                                break
                            # catchall — домен принимает любые адреса
                            if result.get("code") == "catchall":
                                item.found_email = email_to_check
                                item.status = ItemStatus.DONE
                                found_email = email_to_check
                                break
                            ...
                            logger.debug(
                                "verify_email failed for %s@%s: %s",
                                item.nickname, domain, exc
                            )
                    if found_email:
                        # Помечаем остальные items того же ad как DONE без email
                        for other in ad_items:
                            if other is not item:
                                other.status = ItemStatus.DONE
                                other.found_email = ""
                        break
                if not found_email:
                    # Помечаем все items как DONE без email
                    for item in ad_items:
                        item.status = ItemStatus.DONE
                return bool(found_email)

        async def _one_ad(args):
            """Обёртка для asyncio.gather — обновляет статистику."""
            nonlocal found, done, si, last_ui, last_commit
            ad_id, ad_items = args
            success = await _process_ad(ad_id, ad_items)
            ...
            async with lock:
                ...
                if now - last_ui >= 2.0:
                    ...
                    await self._update_message(
                        ...
                        f"Объявлений: {done}/{display_total}\n"
                        f"✅ Найдено: {found}\n"
                        f"⚡ {speed:.1f} об/сек"
                        )
        ...
        # ✅ Запускаем по одному таску на ОБЪЯВЛЕНИЕ, а не на каждый item.
        # Это даёт дедупликацию: если нашли email для объявления, остальные
        # вариации того же объявления не проверяем.
        tasks = [asyncio.create_task(_one_ad(args)) for args in items_by_ad]
