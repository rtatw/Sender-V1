...
async def load_all_health_from_db(user_id: int) -> int:
    """✅ MED-29 fix: bulk-load EmailHealth для ВСЕХ аккаунтов пользователя
    одним запросом. Возвращает количество загруженных записей.

    Вызывать в on_startup и в Mailer._do_mail перед рассылкой.
    """
    from database.engine import async_session
    from database.models import EmailHealth, EmailAccount
    from sqlalchemy import select

    try:
        async with async_session() as s:
            # Один SELECT — все EmailHealth для пользователя + join с EmailAccount
            rows = list(await s.scalars(
                select(EmailHealth).where(EmailHealth.user_id == user_id)
            ))
        ...
        loaded = 0
        # Заполняем реестр из загруженных rows
        for row in rows:
            if row.email not in account_emails:
                continue  # аккаунт удалён — пропускаем
            h = _health_registry.get(row.email)
            if h is None:
                h = AccountHealth(email=row.email)
                _health_registry[row.email] = h
            _health_user_map[row.email] = user_id
            h.sends_today = row.sends_today
            h.sends_this_hour = row.sends_this_hour
            h.last_send_ts = row.last_send_ts
            h.hour_window_start = row.hour_window_start
        ...
        # Для аккаунтов без записи в EmailHealth — создаём
        existing_emails = {r.email for r in rows}
        new_records = []
        for acc in accounts:
            if acc.email not in existing_emails:
                h = AccountHealth(email=acc.email)
                h.created_at = time.time()
                _health_registry[acc.email] = h
                _health_user_map[acc.email] = user_id
                new_records.append(EmailHealth(
                    user_id=user_id, email=acc.email,
                    created_at_ts=int(h.created_at),
                ))
                loaded += 1  # ✅ считаем и созданные записи
        if new_records:
            async with async_session() as s:
        ...
        logger.warning("load_all_health_from_db failed for user %s: %s", user_id, e)
        return 0
